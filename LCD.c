...
Embedded Systems Software - Lab #3
Modified by: Mitch Verbuyst
						 Feb 14, 2018

Both main.c and LCD.c files were provided to start this lab, along with any necessary header files.  Both files 
that were used in lab #2 were used as the starting point for this lab, and modified accordingly.
...
The following pins were configured for the Keypad:

	GPIOA - pins 1,2,3,5 (for input)
	GPIOE - pins 10,11,12,13 (for output)

The program is able to read what keypad button is being pressed and display it on the LCD.

Pressing the '#' key on the keypad will clear the entire LCD (as "something cool")
...
//MASK and values for initialzing the keypad
#define Key_E_MODE_MASK 0xF00FFFFF
#define Key_E_MODE 0x05500000
#define Key_A_MODE_MASK 0xFFFFF303
#define Key_A_MODE 0x00000000
...
void LCD_PIN_Init(void){
	...
	RCC->AHB2ENR |= RCC_AHB2ENR_GPIOCEN; //enable GPIO B clock
	RCC->AHB2ENR |= RCC_AHB2ENR_GPIODEN; //enable GPIO B clock
	RCC->AHB2ENR |= RCC_AHB2ENR_GPIOEEN; //enable GPIO B clock 
  ...
  GPIOB->AFR[1] &= B_AFR1_MASK;
	...
	/**************************************KEYPAD***************************************/
		//set GPIOE as output
	GPIOE->MODER &= Key_E_MODE_MASK;
	GPIOE->MODER |= Key_E_MODE;
	
	//set GPIOA as input
	GPIOA->MODER &= Key_A_MODE_MASK;
	GPIOA->MODER |= Key_A_MODE;
...
void LCD_Configure(void){
	...
	LCD->CR = 0x014D; //configure CR 
	LCD->FCR = 0x03C5C70; //configure FCR
...
unsigned char keypad_scan(void){
	...
	unsigned char key;
	uint16_t row[4] = {0x3800, 0x3400, 0x2C00, 0x1C00}; //set row outputs
	uint16_t col[4] = {2UL, 4UL, 8UL, 32UL}; //set column inputs
	uint8_t r,c=0; //counters
	...
	while(1)
	{
		...
		for (r=0; r<4; r++)
		{
			for (i = 0; i<10000;i++){}; //deboucning delay
			for (c=0;c<4;c++)
			{
				GPIOE->ODR = row[r];
				GPIOA->IDR = col[c];
				
				for (i = 0; i<50000;i++){}; //debouncing delay
				
					if ((GPIOA->IDR & col[c]) == 0)
					{
						key = key_map[r][c];
						return key;
					}
